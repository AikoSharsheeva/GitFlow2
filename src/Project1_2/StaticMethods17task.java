...
public class StaticMethods17task {
...
//Create banking account for john doe
        Account johnDoeChecking = new Account("12345678", 100);
        Account johnDoeSavings = new Account("454433", 200);
//Store all  john doe accounts on a list
        ...
        //Ellon Musk
        Account ellonChecking = new Account("11111", 300);
        Account ellonSavings = new Account("2222", 400);
        ...
        //Jessica
        Account jessicaChecking = new Account("8888888", 500);
        Account jessicaSavings = new Account("77777", 600);
...
// Получение списка клиентов банка
        ...
        double result=depositFunds(chaseBank,"Elon Musk","11111",1000);
    ...
    public static double depositFunds(Bank bank, String customerName, String accountNum, double amount) {
        ...
        for (Customer custObj : customerList) {
            if (custObj.getName().equalsIgnoreCase(customerName)) {
                List<Account> accountList=custObj.getAccountsList();
                for(Account accountObj:accountList){
