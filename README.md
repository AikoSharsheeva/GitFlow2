...
Для создания подкласса на основе суперкласса используется ключевое слово **extends**.
Пример:
java
Copy code
...
Подкласс может переопределить (override) методы суперкласса, чтобы предоставить свою собственную реализацию.
Для переопределения метода используется аннотация @Override перед методом в подклассе.
Пример:
java
Copy code
@Override
...
Конструктор подкласса должен вызывать конструктор суперкласса с помощью super() в первой строке.
Пример:
java
Copy code
public SubClass() {
